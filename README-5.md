...
system, built and tested in [Wokwi](https://wokwi.com/projects/470342259908542465).
...
| Potentiometer      | GPIO 34 (analog) |
| Green LED          | GPIO 25 |
| Yellow LED         | GPIO 26 |
| Red LED            | GPIO 27 |
| Buzzer             | GPIO 14 |
| Confirm/Return button | GPIO 12 |
...
1. Clone the repo.
2. Copy `secrets.h.example` to `secrets.h` and fill in your own WiFi and
   [Adafruit IO](https://io.adafruit.com/) credentials. **Never commit
   `secrets.h`** — it's already in `.gitignore`.
3. Create an Adafruit IO feed named `dosage` and a slider block (0–100) for
   your dashboard.
4. Open the project in [Wokwi](https://wokwi.com/) (or push to a GitHub repo
...
5. Install the libraries listed in `libraries.txt` if building on real
